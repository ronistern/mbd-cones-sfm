:- module('sysDiag_smart_weak_optimistic', [ diagnostics_smart_weak_optimistic/3 ]).
...
/******* Main Method *******/
/***************************/
%diagnostics_smart_weak_optimistic(+,+,+).
diagnostics_smart_weak_optimistic(SysFile, System, Obs) :-
	...
	filter_cones_by_classification(SysFile, ObsNum, AllCones, SmartCones),%writeln(SmartCones),
	length(SmartCones,SmartConesLen),
	
	format('Number of initial cones: ~w\n', [SmartConesLen]),
	
	
	writef("------\n**System Info.**\nSystem file: %w, System ID: %w, Obs#: %w\n", [SysFile, SysID, ObsNum]),
	writef("|Gates|: %w, |All Cones|: %w\n", [GatesCnt, SmartConesLen]),
	sysInfoToFile("smartweak", SysFile, ObsNum, SmartConesLen),
	
	getFinalConesFile(SysFile, "smartweak", ObsNum, FinalConesFile),
	getBeeResultsFile(SysFile, "smartweak", ObsNum, BeeResultsFile),
	
	%Step 1 - find MC
	get_time(T0),
	length(Gates, UpBound),
	validMCdiag(Gates, SmartCones, ObsIO, UpBound, _MCdiag, MC, FinalCones,FinalConesFile, BeeResultsFile),
	%writeln('*********************'),
	length(FinalCones, FinalConesCnt),
	
	ConesRemoved is SmartConesLen - FinalConesCnt,

	get_time(T1),
	MCTime is T1 - T0,
	mcToFile("smartweak", SysFile, ObsNum, 0, ConesRemoved, MC, MCTime),
	writef("------\n**MC**\nMC: %w, MC time: %w\n", [MC, MCTime]),
