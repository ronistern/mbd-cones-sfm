:- module('sysDiag_smart_strong_optimistic', [ diagnostics_smart_strong_optimistic/3 ]).
...
/******* Main Method *******/
/***************************/
%diagnostics_smart_strong_optimistic(+,+,+).
diagnostics_smart_strong_optimistic(SysFile, System, Obs) :-
	...
	sysInfoToFile("smartstrong", SysFile, ObsNum, SmartConesLen),
	
	getFinalConesFile(SysFile, "smartstrong", ObsNum, FinalConesFile),
	getBeeResultsFile(SysFile, "smartstrong", ObsNum, BeeResultsFile),
	
	%step 0 - filter cones by classification
	filter_cones_by_classification(Gates, AllCones, SmartCones),
	length(SmartCones,SmartConesLen),
	format('Number of initial cones: ~w\n', [SmartConesLen]),
